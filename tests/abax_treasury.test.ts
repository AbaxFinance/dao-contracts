...
import { ABAX_DECIMALS, ContractRole, ONE_DAY, ONE_YEAR } from 'tests/consts';
...
import { testAccessControlForMessage } from './misc';
...
describe.only('Abax Treasury tests', () => {
  ...
  describe('after deployment', () => {
    it('governor should have DEFAULT_ADMIN role', async () => {
      expect(await treasury.query.hasRole(ContractRole.DEFAULT_ADMIN, governor.address)).to.haveOkResult(true);
    });
    it('governor should have SPENDER role', async () => {
      expect(await treasury.query.hasRole(ContractRole.SPENDER, governor.address)).to.haveOkResult(true);
    });
    it('foundation should have EXECUTOR role', async () => {
      expect(await treasury.query.hasRole(ContractRole.EXECUTOR, foundation.address)).to.haveOkResult(true);
    });
    it('foundation should have CANCELLER role', async () => {
      expect(await treasury.query.hasRole(ContractRole.CANCELLER, foundation.address)).to.haveOkResult(true);
    });
    it('foundation should not have SPENDER  and DEFAULT_ADMIN role', async () => {
      expect(await treasury.query.hasRole(ContractRole.SPENDER, foundation.address)).to.haveOkResult(false);
      expect(await treasury.query.hasRole(ContractRole.DEFAULT_ADMIN, foundation.address)).to.haveOkResult(false);
    });
    it('vesting contract should not have any role', async () => {
      expect(await treasury.query.hasRole(ContractRole.DEFAULT_ADMIN, foundation.address)).to.haveOkResult(false);
      expect(await treasury.query.hasRole(ContractRole.SPENDER, foundation.address)).to.haveOkResult(false);
      expect(await treasury.query.hasRole(ContractRole.EXECUTOR, foundation.address)).to.haveOkResult(false);
      expect(await treasury.query.hasRole(ContractRole.CANCELLER, foundation.address)).to.haveOkResult(false);
    });
  ...
  //   }));
  // });
  describe.only('while in the vault there is 100 tokenA, 100tokenB, 100tokenC and 100 native token', () => {
    beforeEach(async () => {
    ...
    describe('while create order is called by the SPENDER', () => {
      ...
      it('should create many order with one operation ', async () => {
