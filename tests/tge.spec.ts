...
import { describe } from 'tests/make-suite';
...
describe(
  ...
  (getTestEnv) => {
    ...
    describe(
      'With another token deployed ',
      (getTestEnv2) => {
        it('Has deployed succesfully', async () => {
          const contracts = getTestEnv2().contracts;
          const abaxName = (await contracts.abaxToken.query.tokenName()).value.ok;
          expect(abaxName).to.equal('ABAX');
          const someTokenName = (await contracts.someToken.query.tokenName()).value.ok;
          expect(someTokenName).to.equal('ABAX2');
        });
      },
      async (api: ApiPromise) => {
        //
        const previousContracts = getTestEnv().contracts;
        const someToken = await deployEmitableToken(api, owner, 'ABAX2', ABAX_DECIMALS);

        return { ...previousContracts, someToken };
      },
    );
