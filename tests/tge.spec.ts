...
import { AccessControlError, TGEErrorBuilder } from 'typechain/types-returns/abax_tge';
...
makeSuite('TGE', prepareEnvBase, (getTestEnv) => {
  ...
  describe('phase 1', () => {
    describe('initialization/access control', () => {
      ...
      it.only('Only admin can set bonus multiplier', async () => {
        const contributor = contributors[0];
        ...
        // expect(queryRes.value.err).to.deep.equal(TGEErrorBuilder.AccessControlError(AccessControlError.missingRole));
    ...
    describe('user has bonus', () => {
      it('Gets the correct amount of tokens & vesting', async () => {
        ...
        expect(queryPreMultiplier.value.ok?.ok?.toString()).to.equal(new BN(40).mul(ABAX_DENOM).toString());
        ...
        const expectedTokensReceivedTotal = new BN(40).mul(ABAX_DENOM).mul(bonusMultiplierE6).div(E6bn);
  ...
  describe('phase 2', () => {
    beforeEach(async () => {
      await setBlockTimestamp(getTestEnv().api, parseInt(tgeStartTime) + 100);
      await abaxToken.tx.mint(tge.address, phaseOneTokenCap);
      const tx = tge.withSigner(admin).tx.contribute({ value: phaseOneTokenCap.muln(20).divn(100).mul(E6bn).div(phaseOneAmountPerMilllionTokens) });
      await expect(tx).to.eventually.be.fulfilled;
      await setBlockTimestamp(getTestEnv().api, parseInt(tgeStartTime) + 100);
      const currentParams = await getTgeParams(tge);
      expect(currentParams.phaseTwoStartTime.toString()).to.equal((parseInt(tgeStartTime) + 100).toString());
    ...
    describe('user has no bonus', () => {
      it('Gets the correct amount of tokens & vesting', async () => {
        ...
        const queryPostMultiplierSet = await tge.withSigner(contributor).query.contribute({ value: 1 });
        const expectedTokensReceivedTotal = new BN(40).mul(ABAX_DENOM);
        expect(queryPostMultiplierSet.value.ok?.ok?.toString()).to.equal(expectedTokensReceivedTotal.toString());
