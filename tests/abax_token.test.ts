...
describe('AbaxToken', () => {
  ...
  describe('generate', () => {
    ...
    describe('when called by generator', () => {
      ...
      const inflationChange = amount.div(ONE_YEAR.muln(10));
      let capBefore;
      ...
      describe('single generate', () => {
        ...
        it('should increase cap', async () => {
          await tx;
          await expect(await abaxToken.query.cap()).to.haveOkResult(capBefore.add(amount));
        });
        it('should increase inflation_rate_per_milisecond', async () => {
          await tx;
          await expect(await abaxToken.query.inflationRatePerMilisecond()).to.haveOkResult(inflationRatePerMilisecondBefore.add(inflationChange));
        });
      });
      describe('when called multiple times', () => {
        ...
        it('should increase cap', async () => {
          //freeze time
          await time.setTo(await time.latest());

          //1
          capBefore = (await abaxToken.query.cap()).value.ok!;
          await abaxToken.withSigner(generator).tx.generate(other.address, amount);
          await expect(await abaxToken.query.cap()).to.be.haveOkResult(capBefore.add(amount));
          //2
          capBefore = (await abaxToken.query.cap()).value.ok!;
          await abaxToken.withSigner(generator).tx.generate(other.address, amount);
          await expect(await abaxToken.query.cap()).to.be.haveOkResult(capBefore.add(amount));
        ...
        it('should increase inflation_rate_per_milisecond', async () => {
          ...
          inflationRatePerMilisecondBefore = (await abaxToken.query.inflationRatePerMilisecond()).value.ok!;
          await abaxToken.withSigner(generator).tx.generate(other.address, amount);
          await expect(await abaxToken.query.inflationRatePerMilisecond()).to.be.haveOkResult(inflationRatePerMilisecondBefore.add(inflationChange));
          //2
          inflationRatePerMilisecondBefore = (await abaxToken.query.inflationRatePerMilisecond()).value.ok!;
          await abaxToken.withSigner(generator).tx.generate(other.address, amount);
          await expect(await abaxToken.query.inflationRatePerMilisecond()).to.be.haveOkResult(inflationRatePerMilisecondBefore.add(inflationChange));
    ...
    describe('when called by minter', () => {
      ...
      describe('when called after token generation and YEAR has passed', () => {
        ...
        it('minting 0 amount should inflate cap', async () => {
          ...
          await expect(await abaxToken.query.cap()).to.haveOkResult(capBefore.add(INFLATION_PER_YEAR));
        });
    ...
    describe('should persist state after upgrade', async () => {
      ...
      describe('after upgrade deployment checks should pass', () => {
        it(`has set roles correctly`, async () => {
          await expect(abaxToken.query.hasRole(stringToSelectorId('CODE_UPGRADER'), upgrader.address)).to.haveOkResult(true);
          await expect(abaxToken.query.hasRole(stringToSelectorId('MINTER'), minter.address)).to.haveOkResult(true);
          await expect(abaxToken.query.hasRole(stringToSelectorId('GENERATOR'), generator.address)).to.haveOkResult(true);
        });
        it('should have correct name', async () => {
          await expect(await abaxToken.query.tokenName()).to.haveOkResult('NAME');
        });
        it('should have correct symbol', async () => {
          await expect(await abaxToken.query.tokenSymbol()).to.haveOkResult('SYMBOL');
        });
        it('should have correct decimals', async () => {
          await expect(await abaxToken.query.tokenDecimals()).to.haveOkResult(ABAX_DECIMALS);
        });
        it('should have 10_000 total supply', async () => {
          await expect(await abaxToken.query.totalSupply()).to.haveOkResult(10000);
        });
        it('should have cap 0', async () => {
          await expect(await abaxToken.query.cap()).to.haveOkResult(0);
        });
        it('should have inflation_rate_per_milisecond 0', async () => {
          await expect(await abaxToken.query.inflationRatePerMilisecond()).to.haveOkResult(0);
        });
      });
      ...
      describe('custom checks', () => {
        ...
        it('should contain new fields', async () => {
          ...
          await expect(await abaxTokenAsV2.query.getNewField()).to.haveOkResult({ a: 5, b: 0 });
...
function deploymentChecks(getCtx: () => { abaxToken: AbaxToken }) {
  ...
  it('should have correct name', async () => {
    await expect(await getCtx().abaxToken.query.tokenName()).to.haveOkResult('NAME');
  });
  it('should have correct symbol', async () => {
    await expect(await getCtx().abaxToken.query.tokenSymbol()).to.haveOkResult('SYMBOL');
  });
  it('should have correct decimals', async () => {
    await expect(await getCtx().abaxToken.query.tokenDecimals()).to.haveOkResult(ABAX_DECIMALS);
  });
  it('should have 0 total supply', async () => {
    await expect(await getCtx().abaxToken.query.totalSupply()).to.haveOkResult(0);
  });
  it('should have cap 0', async () => {
    await expect(await getCtx().abaxToken.query.cap()).to.haveOkResult(0);
  });
  it('should have inflation_rate_per_milisecond 0', async () => {
    await expect(await getCtx().abaxToken.query.inflationRatePerMilisecond()).to.haveOkResult(0);
  });
}
