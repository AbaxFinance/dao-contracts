...
"env CARGO_TERM_COLOR=always FORCE_COLOR=1  npx tsx $SCRIPT_DIR/runWithoutWarnings.ts npx mocha --node-option max-old-space-size=16384 --config ./.mocharc.js -C --exit --full-trace false --require tsx/cjs --require 'tests/globalHooks.ts' 'tests/**/*.ts' --colors"
